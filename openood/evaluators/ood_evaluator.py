...
class OODEvaluator(BaseEvaluator):
    ...
    def eval_ood(self,
        ...
        id_pred, id_gt = postprocessor.acc_inference(
        ...
        # load nearood data and compute ood metrics
        print(u'\u2500' * 70, flush=True)
        self.my_eval_ood(net, [id_pred, id_gt],
                       ood_data_loaders,
                       postprocessor,
                       ood_split='nearood')
    ...
    def my_eval_ood(self,
                  net: nn.Module,
                  id_list: List[np.ndarray],
                  ood_data_loaders: Dict[str, Dict[str, DataLoader]],
                  postprocessor: BasePostprocessor,
                  ood_split: str = 'nearood', id_dataset_name='id_dataset'):
        print(f'Processing {ood_split}...', flush=True)
        [id_pred, id_gt] = id_list
        metrics_list = []

        # if ood_split == 'nearood':
        #     print(f'Performing inference on {id_dataset_name} dataset...')
        #     return
        ...
        for dataset_name, ood_dl in ood_data_loaders[ood_split].items():
            ...
            id_conf, ood_pred, ood_conf, ood_gt = postprocessor.inference(net, ood_dl, dataset_name)
            ood_gt = -1 * np.ones_like(ood_gt)  # hard set to -1 as ood
            if self.config.recorder.save_scores:
                self._save_scores(ood_pred, ood_conf, ood_gt, dataset_name)
                self._save_scores(id_pred, id_conf, id_gt, dataset_name+"_vs_"+id_dataset_name)

            pred = np.concatenate([id_pred, ood_pred])
            conf = np.concatenate([id_conf, ood_conf])
            id_gt = np.ones_like(id_conf)
            ...
            ood_metrics = compute_all_metrics(conf, label, pred[:len(label)])
    ...
    def eval_acc(self,
        ...
        if type(net) is dict:
            ...
            net.eval()
        self.id_pred, self.id_gt = postprocessor.acc_inference(
            net, data_loader)
